...
class EyeTribe:
    ...
    def __init__(self, logfilename='default.txt'):
        ...
        self._hbthread = Thread(target=self._heartbeater,
                                args=[self._heartbeatinterval])
        ...
        self._dpthread = Thread(target=self._process_samples,
                                args=[self._queue])
    ...
    def _log_header(self):
        ...
        header = self._separator.join(['timestamp', 'time', 'fix', 'state',
                                       ...
                                       'Lrawx', 'Lrawy', 'Lavgx', 'Lavgy',
                                       'Lpsize', 'Lpupilx', 'Lpupily',
                                       'Rrawx', 'Rrawy', 'Ravgx', 'Ravgy',
                                       'Rpsize', 'Rpupilx', 'Rpupily'
                                       ])
...
class connection:
    ...
    def create_json(self, category, request, values):
        ...
        # error if the values are anything other than a dict, tuple or list
            ...
            raise Exception(
                "values should be dict, tuple or list, not '%s' (values = %s)" % (
                    type(values), values))
...
class tracker:
    ...
    def get_heartbeatinterval(self):
        ...
        # send the request
        response = self.connection.request('tracker', 'get',
                                           ['heartbeatinterval'])
        ...
        if response['statuscode'] == 200:
            ...
            raise Exception(
                "Error in tracker.get_heartbeatinterval: %s (code %d)" % (
                    response['values']['statusmessage'],
                    response['statuscode']))
    ...
    def get_trackerstate(self):
        ...
        if response['statuscode'] == 200:
            ...
            raise Exception(
                "Error in tracker.get_trackerstate: %s (code %d)" % (
                    response['values']['statusmessage'],
                    response['statuscode']))
    ...
    def get_iscalibrated(self):
        ...
        if response['statuscode'] == 200:
            ...
            raise Exception(
                "Error in tracker.get_iscalibrated: %s (code %d)" % (
                    response['values']['statusmessage'],
                    response['statuscode']))
    ...
    def get_iscalibrating(self):
        ...
        if response['statuscode'] == 200:
            ...
            raise Exception(
                "Error in tracker.get_iscalibrating: %s (code %d)" % (
                    response['values']['statusmessage'],
                    response['statuscode']))
        ...
        # if this was the final calibration point, return the results
            ...
            for pointdict in response['values']['calibresult']['calibpoints']:
                returndict['calibpoints'].append({'state': pointdict['state'],
                                                  'cpx': pointdict['cp']['x'],
                                                  'cpy': pointdict['cp']['y'],
                                                  'mecpx': pointdict['mecp'][
                                                      'x'],
                                                  'mecpy': pointdict['mecp'][
                                                      'y'],
                                                  ...
                                                  'Racd': pointdict['acd'][
                                                      'adr'],
                                                  'mepix': pointdict['mepix'][
                                                      'mep'],
                                                  'Lmepix': pointdict['mepix'][
                                                      'mepl'],
                                                  'Rmepix': pointdict['mepix'][
                                                      'mepr'],
                                                  'asdp': pointdict['asdp'][
                                                      'asd'],
                                                  'Lasdp': pointdict['asdp'][
                                                      'asdl'],
                                                  'Rasdp': pointdict['asdp'][
                                                      'asdr']
                                                  })
            return returndict
    ...
    def get_frame(self):
        ...
        return {'timestamp': response['values']['frame']['timestamp'],
                ...
                'psize': (response['values']['frame']['lefteye']['psize'] +
                          response['values']['frame']['righteye'][
                              'psize']) / 2.0,
                ...
                'Lpupilx': response['values']['frame']['lefteye']['pcenter'][
                    'x'],
                'Lpupily': response['values']['frame']['lefteye']['pcenter'][
                    'y'],
                ...
                'Rpupilx': response['values']['frame']['righteye']['pcenter'][
                    'x'],
                'Rpupily': response['values']['frame']['righteye']['pcenter'][
                    'y']
                }
    ...
    def set_push(self, push=None):
        ...
        # send the request
        response = self.connection.request('tracker', 'set',
                                           {'push': str(self.push).lower()})
        # return value or error
        if response['statuscode'] == 200:
            return self.push
        else:
...
class calibration:
    ...
    def pointstart(self, x, y):
        ...
        # send the request
        response = self.connection.request('calibration', 'pointstart',
                                           {'x': x, 'y': y})
        # return value or error
        if response['statuscode'] == 200:
            return True
